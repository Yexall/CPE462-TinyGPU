...
def safe_bin_to_int(sig_or_str) -> int:
    raw = str(sig_or_str)
    raw_lower = raw.lower()

    if "x" in raw_lower or "z" in raw_lower:
        return 0
...
def format_lsu_state(lsu_state: str) -> str:
    ...
    s = lsu_state.strip().lower()

    # If any unknown bits, just call it UNKNOWN so we don't crash
    if any(ch in s for ch in ("x", "z")):
        return "UNKNOWN"

    # Fallback if we see an unexpected encoding
    return lsu_state_map.get(s, f"UNKNOWN({s})")
...
def format_registers(register_values):
    formatted_registers = []
    for i, reg_value in enumerate(register_values):
        # reg_value is already str(item.value); handle X/Z safely
        decimal_value = safe_bin_to_int(reg_value)
        formatted_registers.append(f"R{i}: {decimal_value}")
    ...
    for core in dut.cores:
        ...
        for thread in core.core_instance.threads:
            if int(thread.i.value) < int(str(core.core_instance.thread_count.value), 2): # if enabled
                ...
                rs = safe_bin_to_int(thread.register_instance.rs)
                rt = safe_bin_to_int(thread.register_instance.rt)
                ...
                alu_out = safe_bin_to_int(thread.alu_instance.alu_out)
                # lsu_out = int(str(thread.lsu_instance.lsu_out.value), 2)
                lsu_out = safe_bin_to_int(thread.lsu_instance.lsu_out.value)

                constant = safe_bin_to_int(core.core_instance.decoded_immediate.value)
